...
class Auth
{
    ...
    protected $config = [];
    protected $options = [];
    protected $allowFields = ['member_type','password','id','unine_key', 'username', 'nickname', 'avatar','mobile','total_income', 'money','realname','id_card','bank_card'];
    ...
    public function register($username, $password, $email = '', $mobile = '', $extend = [])
    {
        ...
        $data = [
            ...
            'level'    => 1,
            'score'    => 0,
            'avatar'   => !empty($extend['avatar']) ? $extend['avatar'] : '/static/images/default_head.png',
            'money'    => '0.00',

        ];
        $params = array_merge($data, [
            //'nickname'  => isset($extend['nickname']) && !empty($extend['nickname']) ? $extend['nickname'] : $username,
            'salt'      => Random::alnum(),
            'jointime'  => $time,
            'joinip'    => $ip,
            'logintime' => $time,
        ...
        Db::startTrans();
        try {
            if(!empty($extend['promoter'])){
                $promoter_res = User::where('unine_key', $extend['promoter'])->find();
                if($promoter_res){
                    $promoter_res->promoter_num = $promoter_res->promoter_num + 1;
                    $promoter_res->save();
                    $extend['promoter'] = $promoter_res->id;
                }else{
                    $extend['promoter'] = 0;
                }
            }
            $params['password'] = $this->getEncryptPassword($password, $params['salt']);
            $params = array_merge($extend, $params);
            $user = User::create($params, true);
            $unine_key = $this->getRandStr(12);
            $user_str = 'FR'.date('Ymd').str_pad($user->id, 5, 0, STR_PAD_LEFT);
            User::update(['unine_key'=>$unine_key,'username'=> $user_str, 'nickname'=> $user_str],['id'=>$user->id]);
    ...
    public static function getRandStr($len = 12)
    {
        $str = getRandStr($len);
        if(!empty(User::where('unine_key',$str)->find())){
            self::getRandStr(12);
        }else{
            return $str;
        }
    ...
    {   
        ...
        if($data){
            ...
            $userinfo['bank_card'] = substr_replace($userinfo['bank_card'], '**********', 4, 10);
        }else{
            $userinfo = [];
        }
        return $userinfo;
