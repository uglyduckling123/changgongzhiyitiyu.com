...
use app\common\model\IntegralConfig as integralConfigModel;
use think\Cache;
use think\Db;
use think\Exception;
use think\Validate;
...
class Config extends Backend
{
    ...
    public function _initialize()
    {
        ...
        $this->model = new integralConfigModel;
        integralConfigModel::event('before_write', function ($row) {
    ...
    {
        $siteList = [];
        $groupList = integralConfigModel::getGroupList();
        ...
        $this->view->assign('typeList', integralConfigModel::getTypeList());
        $this->view->assign('ruleList', integralConfigModel::getRegexList());
        $this->view->assign('groupList', integralConfigModel::getGroupList());
        ...
        if ($this->request->isPost()) {
            ...
            if ($params) {
                ...
                if (in_array($params['type'], ['select', 'selects', 'checkbox', 'radio', 'array'])) {
                    $params['content'] = json_encode(integralConfigModel::decode($params['content']), JSON_UNESCAPED_UNICODE);
                } else {
                    $params['content'] = '';
                }
                try {
                ...
                if ($result !== false) {
                    try {
                        integralConfigModel::refreshFile();
                    } catch (Exception $e) {
                        $this->error($e->getMessage());
                    }
                    $this->success();
                ...
                foreach ($this->model->all() as $v) {
                    if (isset($row[$v['name']])) {
                        ...
                        if (is_array($value) && isset($value['field'])) {
                            $value = json_encode(integralConfigModel::getArrayData($value), JSON_UNESCAPED_UNICODE);
                        } else {
    ...
    public function del($ids = "")
    {
        ...
        $config = integralConfigModel::getByName($name);
        if ($name && $config) {
            try {
                $config->delete();
                integralConfigModel::refreshFile();
